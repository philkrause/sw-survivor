...
export class EnemySystem {
  ...
  private enemyTypes = ['storm', 'soldier1'];
 ...
 */
  ...
  private spawnEnemy(): void {
    ...
    if (this.player.getLevel() > 2)
      type = Phaser.Utils.Array.GetRandom(this.enemyTypes); // "storm" or "soldier1"
  ...
  private configureEnemyProperties(enemy: Phaser.Physics.Arcade.Sprite): void {
    ...
    //enemy.setCollideWorldBounds(true); // Optional: for collision with world bounds
    //enemy.setGravity(0); // Optional: prevent gravity effect
  }
  ...
  update(_time: number, _delta: number): void {
    ...
    for (const enemy of this.activeEnemies) {
      ...
      if (Phaser.Geom.Rectangle.Contains(this.cameraRect, enemy.x, enemy.y)) {
        ...
        // Upd      //Flip sprite based on direction
